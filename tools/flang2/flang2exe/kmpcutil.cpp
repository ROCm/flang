 ...
 *
 * Added code support for openmp schedule clause
...
mp_sched_to_kmpc_sched(int sched)
{
  ...
  switch (sched) {
  case SCHED_PREFIX(AUTO) | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(AUTO):
    return KMP_SCH_AUTO;
  case SCHED_PREFIX(DYNAMIC):
    return KMP_SCH_DYNAMIC_CHUNKED;
  case SCHED_PREFIX(GUIDED) | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(GUIDED):
    return KMP_SCH_GUIDED_CHUNKED;
  case SCHED_PREFIX(RUNTIME) | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(RUNTIME):
    return KMP_SCH_RUNTIME_CHUNKED;
  case SCHED_PREFIX(STATIC) | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(STATIC):
    return KMP_SCH_STATIC;
  case SCHED_PREFIX(DIST_STATIC) | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(DIST_STATIC):
    return KMP_DISTRIBUTE_STATIC;
  case SCHED_PREFIX(DYNAMIC) | MP_MOD_NONMONOTONIC:
    return KMP_SCH_STATIC_STEAL;

  case SCHED_PREFIX(RUNTIME) | MP_MOD_SIMD:
    return KMP_SCH_RUNTIME_SIMD;
  case SCHED_PREFIX(GUIDED) | MP_MOD_SIMD:
    return KMP_SCH_GUIDED_SIMD;
  case SCHED_PREFIX(STATIC) | MP_MOD_SIMD:
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_CHUNKED | MP_MOD_SIMD:
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_CHUNKED | MP_SCH_CHUNK_1 | MP_MOD_SIMD:
  case SCHED_PREFIX(STATIC) | MP_SCH_CHUNK_1 | MP_MOD_SIMD:
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_CHUNKED | MP_SCH_BLK_CYC | MP_MOD_SIMD:
  case SCHED_PREFIX(STATIC) | MP_SCH_BLK_CYC | MP_MOD_SIMD:
  case SCHED_PREFIX(STATIC) | MP_SCH_BLK_ALN | MP_MOD_SIMD:
    return KMP_SCH_UPPER; 

  /* Ordered */
  case SCHED_PREFIX(AUTO) | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(AUTO) | MP_SCH_ATTR_ORDERED:
    return KMP_ORD_AUTO;
  case SCHED_PREFIX(RUNTIME) | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(RUNTIME) | MP_SCH_ATTR_ORDERED:
    return KMP_ORD_RUNTIME;
  case SCHED_PREFIX(STATIC) | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_ORDERED:
    return KMP_ORD_STATIC;
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_ORDERED | MP_SCH_ATTR_CHUNKED | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_ORDERED | MP_SCH_ATTR_CHUNKED:
    return KMP_ORD_STATIC_CHUNKED;
  case SCHED_PREFIX(DYNAMIC) | MP_SCH_ATTR_ORDERED | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(DYNAMIC) | MP_SCH_ATTR_ORDERED | MP_SCH_ATTR_CHUNKED | MP_MOD_MONOTONIC:
  ...
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_CHUNKED | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_CHUNKED | MP_SCH_CHUNK_1 | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_CHUNK_1 | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_CHUNKED | MP_SCH_BLK_CYC | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_BLK_CYC | MP_MOD_MONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_BLK_ALN | MP_MOD_MONOTONIC:
    return KMP_ORD_STATIC_CHUNKED;

  /* Special cases of static */
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_CHUNKED | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_CHUNKED | MP_SCH_CHUNK_1 | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_CHUNK_1 | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_ATTR_CHUNKED | MP_SCH_BLK_CYC | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_BLK_CYC | MP_MOD_NONMONOTONIC:
  case SCHED_PREFIX(STATIC) | MP_SCH_BLK_ALN | MP_MOD_NONMONOTONIC:
