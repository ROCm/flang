...
struct key_value *cc_ptr_for_pmembers = NULL;

/** \brief  To map a pointer member of a structure to target, a suitable
 *          compiler created pointer is returned
 *
 * \param base_sptr symbol table pointer of the structure variable
 * \param mem_sptr  symbole table pointer of the member of structure
 *                  from structure definition
 * \param ast
 *
 * \returns         ast of a pointer that can to assigned to member
 *                  of the structure
 */
int get_cc_pointer(int base_sptr, int mem_sptr)
{
    int psym, dtype, past;
    int oldndim, oldndefer;
    static i = 0, N = 2;

    if(!cc_ptr_for_pmembers) {
        cc_ptr_for_pmembers = (struct key_value*)malloc(sizeof(struct key_value) * N);
    }

    // check for non pointer member
    if(SCG(mem_sptr) != SC_BASED ||
                        STYPEG(mem_sptr) != ST_MEMBER)
        return 0;

    for(int j=0; j<N; ++j) {
        if((base_sptr + mem_sptr) == cc_ptr_for_pmembers[j].key){
            return cc_ptr_for_pmembers[j].value;
        }
    }

    psym = getcctmp('P', 0, ST_IDENT, DTY(DTYPEG(mem_sptr)+1));

    oldndim = sem.arrdim.ndim;
    oldndefer = sem.arrdim.ndefer;
    dtype = mk_arrdsc();
    sem.arrdim.ndim = oldndim;
    sem.arrdim.ndefer = oldndefer;
    DTY(dtype+1) = DTYPEG(psym);
    DTYPEP(psym, dtype);

    ALLOCP(psym, 1);
    DCLDP(psym, TRUE);
    POINTERP(psym, TRUE);
    STYPEP(psym, ST_ARRAY);

    get_static_descriptor(psym);
    get_all_descriptors(psym);

    past = mk_id(psym);
    if(i == N) {
        N *= 2;
        cc_ptr_for_pmembers = (struct key_value *)realloc(cc_ptr_for_pmembers,
                                        sizeof(struct key_value)*N);
    }
    cc_ptr_for_pmembers[i].key = base_sptr + mem_sptr;
    cc_ptr_for_pmembers[i++].value = past;
    return past;
}
