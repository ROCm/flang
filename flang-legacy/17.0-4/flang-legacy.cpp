...
std::string Get_clang_Path(const char *Argv0, bool CanonicalPrefixes) {
  std::string pathString;
  if (!CanonicalPrefixes) {
    SmallString<128> ExecutablePath("clang");
    // Do a PATH lookup if clang isn't a valid path.
    if (!llvm::sys::fs::exists(ExecutablePath))
      if (llvm::ErrorOr<std::string> P =
              llvm::sys::findProgramByName(ExecutablePath))
        ExecutablePath = *P;
    pathString = std::string(ExecutablePath.str());
  } else {
    // This just needs to be some symbol in the binary; C++ doesn't
    // allow taking the address of ::main however.
    void *P = (void *)(intptr_t)Get_clang_Path;
    pathString = llvm::sys::fs::getMainExecutable(Argv0, P);
  }
  return pathString.substr(0, pathString.rfind("/")).append("/clang");
}
...
int main(int Argc, char **Argv) {
  ...
  std::string Path = Get_clang_Path(Args[0], CanonicalPrefixes);
  ...
  if (C && !C->containsError()) {
    ...
    for (auto &Job : C->getJobs()) {
      ...
      std::string cmdname = exec.substr(exec.rfind("/") + 1);
      bool isclang = cmdname.compare("clang") == 0;
      bool isllvmlink = cmdname.compare("llvm-link") == 0;
      // printf("===>ISCLANG:%d cmdname:%s executable:%s\n",isclang, cmdname.c_str(), exec.c_str());
      if (isclang) {
        bool needsModifiedArgList = false;
        for (const char *arg : Job.getArguments()) {
          std::string argstr(arg);
          if (argstr.compare("-fvisibility") == 0 ||
              argstr.compare("--mrelax-relocations") == 0 ||
              argstr.compare("-no-opaque-pointers") == 0) {
            needsModifiedArgList = true;
          }
        }
        if (needsModifiedArgList) {
		printf(" CLANG needs modified arglist\n");
          llvm::opt::ArgStringList ArgList = Job.getArguments();
          llvm::opt::ArgStringList newArgList;
          bool skipnext = false;
          for (const char *arg : Job.getArguments()) {
            std::string argstr(arg);
            if (argstr.compare("-fvisibility") == 0) {
              skipnext = true;
            } else if (argstr.compare("-no-opaque-pointers") == 0) {
              skipnext = false;
            } else if (argstr.compare("--mrelax-relocations") == 0) {
              skipnext = false;
            } else if (skipnext) {
              skipnext = false;
            } else {
          ...
          Job.replaceArguments(std::move(newArgList));
        }
      }
      for (const char *arg : Job.getArguments()) {
        std::string argstr(arg);
