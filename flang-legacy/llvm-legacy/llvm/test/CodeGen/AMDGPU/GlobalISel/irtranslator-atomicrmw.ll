...
define float @test_atomicrmw_fadd(float addrspace(3)* %addr) {
  ...
  ; CHECK-NEXT:   [[ATOMICRMW_FADD:%[0-9]+]]:_(s32) = G_ATOMICRMW_FADD [[PRED_COPY]](p3), [[C]] :: (load store seq_cst (s32) on %ir.addr, addrspace 3)
  ; CHECK-NEXT:   $vgpr0 = PRED_COPY [[ATOMICRMW_FADD]](s32)
...
define float @test_atomicrmw_fsub(float addrspace(3)* %addr) {
  ...
  ; CHECK-NEXT:   [[LOAD:%[0-9]+]]:_(s32) = G_LOAD [[PRED_COPY]](p3) :: (load (s32) from %ir.addr, addrspace 3)
  ...
  ; CHECK-NEXT:   [[ATOMIC_CMPXCHG_WITH_SUCCESS:%[0-9]+]]:_(s32), [[ATOMIC_CMPXCHG_WITH_SUCCESS1:%[0-9]+]]:_(s1) = G_ATOMIC_CMPXCHG_WITH_SUCCESS [[PRED_COPY]](p3), [[PHI1]], [[FSUB]] :: (load store seq_cst seq_cst (s32) on %ir.2, addrspace 3)
  ...
  ; CHECK-NEXT:   $vgpr0 = PRED_COPY [[PHI2]](s32)
  ; CHECK-NEXT:   SI_RETURN implicit $vgpr0
